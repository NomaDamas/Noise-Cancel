...
class TestThreadsScraperLogin:
    ...
    async def test_login_persists_encrypted_session(self, app_config):
        ...
        key_path = data_dir / "threads_session.key"
        session_path = data_dir / "threads_session.enc"
...
class TestThreadsScraperScrapeFeed:
    @pytest.mark.anyio
    async def test_scrape_feed_loads_stored_session(self, app_config):
        ...
        mock_validate.assert_called_once_with(
            key_path=str(data_dir / "threads_session.key"),
            session_path=str(data_dir / "threads_session.enc"),
            ttl_days=7,
        )
    ...
    async def test_scrape_feed_extracts_and_deduplicates_posts(self, app_config):
        ...
        assert all(post.platform == "threads" for post in posts)
...
class TestThreadsScraperHelpers:
    def test_parse_post_element_sets_platform_to_threads(self, app_config):
        from noise_cancel.scraper.threads import ThreadsScraper
...
def test_scraper_registry_has_threads_mapping():
    from noise_cancel.scraper.registry import SCRAPER_REGISTRY
    from noise_cancel.scraper.threads import ThreadsScraper

    assert SCRAPER_REGISTRY.get("threads") is ThreadsScraper
